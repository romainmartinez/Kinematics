function [export] = posthoc_contribution(Y, factor1, factor2, factor3, anova3, string, p, level, delta)
index = 0;
                        ...
                        if ttest.spmi.h0reject == 1
                            ...
                            for icluster = 1 : ttest.spmi.nClusters
                                ...
                                if export(index).cluster{icluster,4} > 0
                                    export(index).cluster{icluster,5} = [num2str(perm.factor1(iPerm,1)) ' > ' num2str(perm.factor1(iPerm,2))];
                                else
                                    export(index).cluster{icluster,5} = [num2str(perm.factor1(iPerm,2)) ' > ' num2str(perm.factor1(iPerm,1))];
                                end
                            end
                        end
                    end
