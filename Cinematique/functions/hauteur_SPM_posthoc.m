...
for i = length(test) : -1 : 1
    ...
    if spmi.h0reject == 1
        for n = 1 : spmi.nClusters
            ...
            % Obtenir le % de diffï¿½rence dans les zones significatives
        ...
        hgsave(gcf, ['\\10.89.24.15\e\Projet_IRSST_LeverCaisse\ElaboratedData\contribution_articulation\figures\height_SPM_raw\' num2str(delta) '\' num2str(test(i,1)) 'vs' num2str(test(i,2))]);
        close(gcf);
    end
